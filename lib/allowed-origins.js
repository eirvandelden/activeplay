...
function socketIoHandshakeGuard(env) {
  var originIsWhitelisted = socketIoOriginGuard(env);

  return function allowHandshake(request, callback) {
    originIsWhitelisted(request.headers.origin, function (error, allowed) {
...
module.exports = {
  ...
  socketIoHandshakeGuard: socketIoHandshakeGuard
};
